...
using SuperPlay.GameX.Server.App.Tests.Shared;
using SuperPlay.GameX.Shared.ApplicationLayer.Requests;
using SuperPlay.GameX.Shared.ApplicationLayer.Requests.Shared;
using SuperPlay.GameX.Shared.DomainLayer.Data;

namespace SuperPlay.GameX.Server.App.Tests
{
    public class GameServerTests : GameServerTestsBase
    {
        ...
        public async Task LoginCommand_ReturnOk()
        {
            ...
            var loginResponse = await gameServer.TryExecuteAsync(new Login(new DeviceId("device1")));
        ...
        {
            var gameServer = await StartGameServer();
            await gameServer.TryExecuteAsync(new Login(new DeviceId("device1")));
        ...
        {
            var gameServer = await StartGameServer();
            var player1 = (await gameServer.ExecuteAsync(new Login(DeviceId.GenerateNew())));
            var loggedInContext = new LoggedInContext(player1);
            await gameServer.ExecuteAsync(new Logout(loggedInContext));

            var updateResourcesResponse = await gameServer.TryExecuteAsync(new UpdateResources(loggedInContext, ResourceType.Coin, new ResourceValue(1)));
        ...
        public async Task UpdateResourcesCommand_AddResources(ResourceType resourceType)
        {
            ...
            await gameServer.ExecuteAsync(new UpdateResources(loggedInContext, resourceType, new ResourceValue(1)));
            var updateResourcesResponse = await gameServer.TryExecuteAsync(new UpdateResources(loggedInContext, resourceType, new ResourceValue(2)));
        ...
        public async Task UpdateResourcesCommand_RemoveResources(ResourceType resourceType)
        {
            ...
            await gameServer.ExecuteAsync(new UpdateResources(loggedInContext, resourceType, new ResourceValue(3)));
            var updateResourcesResponse = await gameServer.TryExecuteAsync(new UpdateResources(loggedInContext, resourceType, new ResourceValue(-2)));
        ...
        public async Task SendGiftCommand_ReturnOk(ResourceType resourceType)
        {
            var gameServer = await StartGameServer();
            var player1 = (await gameServer.ExecuteAsync(new Login(DeviceId.GenerateNew())));
            var player2 = (await gameServer.ExecuteAsync(new Login(DeviceId.GenerateNew())));
            var player1Context = new LoggedInContext(player1);
            var player2Context = new LoggedInContext(player2);
            await gameServer.ExecuteAsync(new UpdateResources(player1Context, resourceType, new ResourceValue(10)));
            await gameServer.ExecuteAsync(new UpdateResources(player2Context, resourceType, new ResourceValue(10)));

            var sendGiftResponse = await gameServer.TryExecuteAsync(new SendGift(player1Context, player2, resourceType, new ResourceValue(3)));

            Assert.IsType<Ok<ResourceValue>>(sendGiftResponse);
            Assert.Equal(new ResourceValue(7), sendGiftResponse.GetResult());

            var playerData1 = await gameServer.ExecuteAsync(new GetMyPlayer(player1Context));
            var playerData2 = await gameServer.ExecuteAsync(new GetMyPlayer(player2Context));
        ...
        public async Task SendGiftCommand_ReturnNotEnoughResourcesException(ResourceType resourceType)
        {
            ...
            var sendGiftResponse = await gameServer.TryExecuteAsync(new SendGift(player1Context, player2, resourceType, new ResourceValue(11)));
        ...
        public async Task SendGiftCommand_ReturnGenericRequestException_WhenSendingToHimself(ResourceType resourceType)
        {
            ...
            var sendGiftResponse = await gameServer.TryExecuteAsync(new SendGift(player1Context, player1, resourceType, new ResourceValue(1)));
            Assert.IsType<NotOk<ResourceValue>>(sendGiftResponse);
        ...
        public async Task SendGiftCommand_SendGiftEvent_IfFriendIsOnline()
        {
            ...
            var player2 = (await gameServer.ExecuteAsync(new Login(DeviceId.GenerateNew()), player2Connection));
            var player1Context = new LoggedInContext(player1);

            await gameServer.ExecuteAsync(new UpdateResources(player1Context, ResourceType.Coin, new ResourceValue(2)));
            await gameServer.ExecuteAsync(new SendGift(player1Context, player2, ResourceType.Coin, new ResourceValue(1)));
        ...
        public async Task SendGiftCommand_DoNotSendGiftEvent_IfFriendIsOffline__UsingLogoutCommand()
        {
            ...
            await gameServer.ExecuteAsync(new Logout(player2Context), player2Connection);
            await gameServer.ExecuteAsync(new SendGift(player1Context, player2, ResourceType.Coin, new ResourceValue(1)));
