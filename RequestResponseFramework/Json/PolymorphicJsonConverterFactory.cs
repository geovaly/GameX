...
public class PolymorphicJsonConverterFactory : JsonConverterFactory
{
    ...
    public PolymorphicJsonConverterFactory()
    {
        List<Assembly> defaultAssemblies = [typeof(IRequest).Assembly];
        RegisterType<RequestException>(GetAllTypesSubclassOf<RequestException>(defaultAssemblies));

    }

    public void AddContracts(IReadOnlyCollection<Assembly> assemblies)
    {
        RegisterType<RequestException>(GetAllTypesSubclassOf<RequestException>(assemblies));
        RegisterType<IRequest>(GetAllTypesImplements<IRequest>(assemblies));
    }

    private static IReadOnlyList<Type> GetAllTypesSubclassOf<T>(IEnumerable<Assembly> assemblies)
    {
        return assemblies
            .SelectMany(assembly => assembly.GetTypes())
            .Where(t => t.IsClass && !t.IsAbstract && t.IsSubclassOf(typeof(T))).ToList();
    }

    private static IReadOnlyList<Type> GetAllTypesImplements<T>(IEnumerable<Assembly> assemblies)
    {
